...
export function AIJourneyQuestion({ onNext, onPrev }: AIJourneyQuestionProps) {
  const { updateUserData, userData } = useOnboarding()
  const [selected, setSelected] = useState(userData.aiJourney || "")
  const [showResponse, setShowResponse] = useState(false)

  const handleSelection = (id: string) => {
    setSelected(id)
    updateUserData({ aiJourney: id })
            ...
            <div className="space-y-6">
              <div className="p-6 bg-blue-50 rounded-lg border border-blue-200">
                <p className="text-blue-900 font-medium text-center">
                  Perfect - you're exactly where {selectedOption?.percentage} of successful professionals started. We'll build from here, at your pace. No judgment, just progress.
                </p>
              </div>
